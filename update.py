    ...
    for model in models:
        ...
        try:
            ...
            if response.get('status') == 'success':
                ...
                print(f"- {model_name} pull status: {response.get('status', 'unknown')}")
...
if __name__ == "__main__":
    ...
    filtered_models = [model for model in models if model not in selected_models]

    print(f"\nModels not updated due to size:")

    for model in filtered_out_models:
