...
if ul:
    ...
    for li in ul.find_all("li"):
        ...
        desc_elem = li.select_one(
            "a > div.flex.flex-col.space-y-2 > p.max-w-md.break-words"
        )
        ...
        downloads_updates_elem = li.select_one(
            "a > div.flex.flex-col.space-y-2 > p.my-2.flex.space-x-5.font-medium.text-neutral-500"
        )
        ...
        if name_elem and desc_elem and tools_sizes_elem and downloads_updates_elem:
            ...
            tools_sizes = [
                ...
                for span in tools_sizes_elem.find_all("span")
            ]

            # Extract and clean downloads and updates
            downloads_updates = [
                clean_text(span.text.strip())
                for span in downloads_updates_elem.find_all("span")
            ]
